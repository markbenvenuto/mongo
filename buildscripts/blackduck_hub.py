...
def _get_supression_field(name, ymap, field: str):
    if field not in ymap:
        return None
...
Build Baron:
A BF ticket should be generated and assigned to "Service Development Platform" with
this text.

Developer:

To address this build failure, the next steps are as follows:
1. Verify if the component is correct in {THIRD_PARTY_COMPONENTS_FILE}.

2. If the component is incorrect, add a comment to the component and mark it as "Ignored". or
...
def _generate_report_missing_directory(mgr: ReportManager, directory: str):
    mgr.write_report(
...
Developer:
To address this build failure, the next steps are as follows:
1. Add the component manually to the project at {BLACKDUCK_PROJECT_URL}.
2. If the component is correct, add the correct information to {THIRD_PARTY_COMPONENTS_FILE}.
...
This is caused by one of two issues:
1. Black Duck has erroroneously detected that MongoDB is vendoring software that it does not.
...
Developer:
To address this build failure, the next steps are as follows:
1. Verify if the component is correct at {BLACKDUCK_PROJECT_URL}.
