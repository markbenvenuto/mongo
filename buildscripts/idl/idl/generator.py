...
def _get_qualified_method_name(name):
    # type: (unicode) -> unicode
    """Get a method name from a fully qualified method name."""
...
def _is_primitive_type(cpp_type):
    ...
    return cpp_type in ['bool', 'double', 'std::int32_t', 'std::int64_t']
...
def _get_field_cpp_type(field):
    ...
    if field.struct_type:
        cpp_type = _camel_case(field.struct_type)
    else:
...
class _CppHeaderFileWriter(_CppFileWriterBase):
    ...
    def gen_class_declaration_block(self, class_name):
        ...
        return _IndentedScopedBlock(self._writer, 'class %s {' % _camel_case(class_name), '};')
    ...
    def gen_getter(self, field):
        ...
        if field.optional and _is_view_type(cpp_type):
            self._writer.write_line('const %s%s get%s() const& { %s }' %
                                    (param_type, optional_ampersand, _camel_case(field.name), body))
            self._writer.write_line("const %s%s get%s() const&& = delete;" %
                                    (param_type, optional_ampersand, _camel_case(field.name)))
        else:
            self._writer.write_line('const %s%s get%s() const { %s }' %
                                    (param_type, optional_ampersand, _camel_case(field.name), body))
        ...
        if _is_view_type(cpp_type):
            if not field.optional:
                self._writer.write_line('void set%s(%s value) { %s = value.%s(); }' %
                                        (_camel_case(field.name), param_type, member_name,
                ...
                with self._block('void set%s(%s value) {' % (_camel_case(field.name), param_type),
                                 "}"):
                    self._writer.write_line(
                        textwrap.dedent("""\
...
class _CppSourceFileWriter(_CppFileWriterBase):
        ...
        with self._predicate(type_predicate):
            ...
            if field.struct_type:
                self._writer.write_line('IDLParserErrorContext tempContext("%s", &ctxt);' %
                                        (field.name))
                self._writer.write_line('auto localObject = element.Obj();')
                self._writer.write_line(
                    'object.%s = %s::parse(tempContext, localObject);' %
                    (_get_field_member_name(field), _camel_case(field.struct_type)))
                ...
                if len(field.bson_serialization_type) == 1 and field.bson_serialization_type[
                    ...
                    self._writer.write_line('object.%s = %s(tempValue);' %
                        ...
                        0] == 'object':
                    method_name = _get_qualified_method_name(field.deserializer)
                    self._writer.write_line('auto localObject = element.Obj();')
    ...
    def gen_deserializer_method(self, struct):
        ...
        with self._block('%s %s::parse(IDLParserErrorContext& ctxt, const BSONObj& bsonObject) {' %
                         (_camel_case(struct.name), _camel_case(struct.name)), '}'):

            self._writer.write_line('%s object;' % _camel_case(struct.name))
    ...
    def generate(self, spec, header_file_name):
        ...
        # Generate include for generated header
        self.gen_include(os.path.abspath(header_file_name))
        self.write_empty_line()
...
def generate_code(spec, header_file_name, source_file_name):
    # type: (ast.IDLAST, unicode, unicode) -> None
    """Generate a C++ header and source file from a idl.ast tree."""

    _generate_header(spec, header_file_name)

    _generate_source(spec, source_file_name, header_file_name)
