...
class _SlowFieldUsageChecker(_FieldUsageCheckerBase):
    ...
    def add_store(self, field_name):
        ...
        with writer.IndentedScopedBlock(self._writer, 'if (MONGO_unlikely(push_result.second == false)) {', '}'):
        ...
        for field in self._fields:
            if (not field.optional) and (not field.ignore) and (not field.chained):
                with writer.IndentedScopedBlock(self._writer,
                                                'if (MONGO_unlikely(usedFields.find(%s) == usedFields.end())) {' %
        ...
        if field.chained:
            ...
            with self._predicate(predicate):
                ...
                if field.chained_struct_field:
                    self._writer.write_line('%s.%s(%s);' % (_get_field_member_name(field.chained_struct_field), _get_field_member_setter_name(field), object_value))
                else:
