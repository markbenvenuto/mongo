...
def _parse_enum(ctxt, spec, name, node):
    ...
    idlenum = syntax.Enum(ctxt.file_name, node.start_mark.line, node.start_mark.column)
    idlenum.name = name

    _generic_parser(ctxt, node, "enum", idlenum, {
        "description": _RuleDesc('scalar', _RuleDesc.REQUIRED),
        "type": _RuleDesc('scalar', _RuleDesc.REQUIRED),
        "values": _RuleDesc('mapping', mapping_parser_func=_parse_values),
    })

    if idlenum.values is None:
        ctxt.add_empty_enum_error(node, idlenum.name)

    spec.symbols.add_enum(ctxt, idlenum)
