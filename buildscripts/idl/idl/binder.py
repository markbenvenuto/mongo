...
def _validate_single_bson_type(ctxt, idl_type, syntax_type):
    ...
    if bson_type == "bindata":
        ...
        ctxt.add_bad_bson_bindata_subtype(idl_type, syntax_type, idl_type.name, bson_type)
    ...
    if len(idl_type.bson_serialization_type) == 1:
        ...
        if bson_type == "any":
            ...
            if idl_type.deserializer is not None and "BSONElement" not in idl_type.deserializer:
                ctxt.add_not_custom_scalar_serialization_not_supported(idl_type, syntax_type,
                                                                       idl_type.name, bson_type)
...
def bind(parsed_spec):
    ...
    """Read a idl.syntax, create a idl.ast tree, and validate the final IDL Specification."""
