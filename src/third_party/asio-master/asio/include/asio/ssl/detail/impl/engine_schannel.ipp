...
engine::engine(SCHANNEL_CRED* context):
  ...
  _handshakeManager(&_hcxt, &_hcred, _serverName, &_inBuffer, &_outBuffer, &_extraBuffer, _pCred),
    ...
    asio::error_code& ec, std::size_t& bytes_transferred)
{
  ...
  if (_state == EngineState::NeedsHandshake || _state == EngineState::InShutdown) {
      ...
      return ssl_want_to_engine(_writeManager.writeUnencryptedData(data.data(), data.size(), bytes_transferred, ec));
  }
}
    ...
    asio::error_code& ec, std::size_t& bytes_transferred)
{
  ...
  if (_state == EngineState::NeedsHandshake) {
      ...
      auto want = ssl_want_to_engine(_readManager.readDecryptedData(data.data(), data.size(), ec, bytes_transferred, &decryptState));
      if (ec) {
          return want;
      }
