...
#define UNIT_TEST_DETAIL_DEFINE_TEST_PRIMITIVE_(FIXTURE_NAME, TEST_NAME, TEST_TYPE, TEST_BASE) \
    ...
    private:                                                                                   \
        ...
        static inline const RegistrationAgent<TEST_TYPE> _agent{#FIXTURE_NAME, #TEST_NAME,  __FILE__};    \
    };                                                                                         \
...
protected:
    ...
    public:
        RegistrationAgent(std::string suiteName, std::string testName, std::string fileName)
            : _suiteName(std::move(suiteName)), _testName(std::move(testName)), _fileName(std::move(fileName)) {
