...
SecureVector<uint8_t> mongo::AzureKMSService::decrypt(ConstDataRange cdr, BSONObj masterKey) {
    ...
    if (obj.hasField("error")) {
        ...
        try {
            azureResponse = AzureKMSError::parse(IDLParserErrorContext("azureDecryptError"),
                                                 obj["error"].Obj());
        ...
        uasserted(5256103,
                  "Azure KMS failed to decrypt: {} : {}"_format(azureResponse.getCode(),
    ...
    size_t raw_len;
    std::unique_ptr<uint8_t,  decltype(std::free)*> raw_str(kms_message_b64url_to_raw(b64Url.c_str(), &raw_len), std::free);
    uassert(5265103, "Azure KMS failed to convert key blob from base64 URL.", raw_str != nullptr);

    std::string blobStr(reinterpret_cast<const char*>(raw_str.get()), raw_len);
...
BSONObj mongo::AzureKMSService::encryptDataKeyByBSONObj(ConstDataRange cdr, BSONObj keyId) {
    ...
    KMSNetworkConnection connection(_sslManager.get());
    auto response = connection.makeOneRequest(keyVaultEndpoint, ConstDataRange(buffer.get(), buffer_len));
    ...
    if (obj.hasField("error")) {
        ...
        uasserted(5256105,
                  "Azure KMS failed to encrypt: {} : {}"_format(azureResponse.getCode(),
                                                              azureResponse.getMessage()));
    }
    ...
    auto b64Url = azureResponse.getValue().toString();
    size_t raw_len;
    // TODO - fat unique ptr, make it thinner?
        std::unique_ptr<uint8_t,  decltype(std::free)*> raw_str(kms_message_b64url_to_raw(b64Url.c_str(), &raw_len), std::free);
    uassert(5265100, "Azure KMS failed to convert key blob from base64 URL.", raw_str != nullptr);
