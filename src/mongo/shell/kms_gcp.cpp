...
public:
   ...
   StringData name() const { return "gcp";}

    static std::unique_ptr<KMSService> create(const GcpKMS&);

    std::vector<uint8_t> encrypt(ConstDataRange cdr, const GcpMasterKey& masterKey);
...
std::vector<uint8_t> GCPKMSService::encrypt(ConstDataRange cdr, const GcpMasterKey& masterKey) {
...
BSONObj GCPKMSService::encryptDataKeyByBSONObj(ConstDataRange cdr, BSONObj keyId) {
    ...
    auto dataKey = encrypt(cdr, masterKey);
