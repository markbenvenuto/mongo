...
TEST_F(ClusterCursorManagerTest, CheckOutCursorPinned) {
    ...
    auto pinnedCursor = getManager()->checkOutCursor(nss, cursorId, nullptr);
    ASSERT_OK(pinnedCursor.getStatus());
    ASSERT_EQ(ErrorCodes::CursorInUse, getManager()->checkOutCursor(nss, cursorId, nullptr).getStatus());
}
...
TEST_F(ClusterCursorManagerTest, CheckOutCursorKilled) {
    ...
    ASSERT_EQ(ErrorCodes::CursorNotFound, getManager()->checkOutCursor(nss, cursorId, nullptr).getStatus());
}
...
TEST_F(ClusterCursorManagerTest, CheckOutCursorUnknown) {
    ASSERT_EQ(ErrorCodes::CursorNotFound, getManager()->checkOutCursor(nss, 5, nullptr).getStatus());
}
