...
StringData FeatureFlag::getVersion() const {
    uassert(1, "getVersion is not supported, Feature is not enabled", _enabled);
    return _version;
}
...
void FeatureFlagServerParameter::append(OperationContext* opCtx,
    ...
    if (enabled) {
        b.append("version", _storage.getVersion());
    }
}
