...
private:
    static void doPost(SharedPromise<std::vector<uint8_t>> shared_promise,
        ...
        try {
            ...
            if (result != CURLE_OK) {
                shared_promise.setError({ErrorCodes::OperationFailed,
                                         str::stream() << "Unexpect error retrieving response: "
                                                       << curl_easy_strerror(result)});
                return;
            }
            
            if (statusCode != 200) {
            shared_promise.setError(Status(ErrorCodes::OperationFailed, str::stream() << "Unexpected http status code from server: " << statusCode));
            return;
            }
