...
Status storeMongodOptions(const moe::Environment& params) {
    ...
    if (params.count("storage.journal.commitIntervalMs")) {
        ...
        // a dev environment.
        storageGlobalParams.journalCommitIntervalMs.store(
            params["storage.journal.commitIntervalMs"].as<int>());
        if (storageGlobalParams.journalCommitIntervalMs.load() < 1 ||
            storageGlobalParams.journalCommitIntervalMs.load() >
