 ...
 */
Status transformImplictOptions(const OptionSection& options,
    ...
    for (const auto& opt : optionDescs) {
        if (!opt._implicit.isEmpty()) {
            ...
            // In this case, the format for the name of the optoin is "long_name,S" where S is a
            // single character.
    ...
    for (size_t i = 0; i < argvOriginal.size(); i++) {
        ...
        do {
            // Skip processing of the last argument in the array
            ...
            if (arg[0] == '-') {
                ...
                if (argvOriginal[i + 1] == "") {
                    ...
                    // If the type of the option was a string type with a implicit value of not "",
                    // then we cannot handle merging these two strings because simply leaving just
                    // the option name around has a different behavior.
...
Status OptionsParser::run(const OptionSection& options,
    ...
    Status ret = transformImplictOptions(options, argvOriginal, &argv);
    if (!ret.isOK()) {
        return ret;
    }
