 ...
 * isMaster from clients. This class also provides static methods for client libraries to create a
 * valid client metadata document.
 *
 * Example document:
 * {
 *    "isMaster" : 1,
 ...
 * document "type" field is required (defaults to "unknown" in Mongo Drivers). The "driver", and
 * "os" documents while required, are for informational purposes only. The content is logged to disk
