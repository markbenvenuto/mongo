...
namespace {
const StringData kMetadataDocumentName = "meta";
...
StatusWith<bool> ClientMetadata::parseIsMasterReply(const BSONObj& doc) {
    bool sawIsMaster = _sawIsMaster;
    _sawIsMaster = true;

    BSONElement element = doc[kMetadataDocumentName];
    if (!element.eoo()) {
        if (sawIsMaster) {
            return Status(ErrorCodes::ClientMetadataDocumentDuplicateDocument,
                          "The client metadata document must only be sent in the first isMaster");
        }

        if (!element.isABSONObj()) {
            return Status(ErrorCodes::TypeMismatch,
                          "The client metadata document must be a document");
        }

        Status s = parseClientMetadataDocument(element.Obj());
        if (!s.isOK()) {
            return s;
        }
...
Status ClientMetadata::parseClientMetadataDocument(const BSONObj& doc) {
    ...
    while (i.more()) {
        ...
        if (name == kApplication) {
            ...
            if (!e.isABSONObj()) {
                return Status(ErrorCodes::TypeMismatch,
                              str::stream() << "The '" << kApplication
                                            << "' field is required to be BSON document in the "
                                               "client metadata document");
            }

            Status s = parseApplicationDocument(e.Obj());
            if (!s.isOK()) {
                return s;
            }
            ...
            if (!e.isABSONObj()) {
                return Status(ErrorCodes::TypeMismatch,
                              str::stream() << "The '" << kDriver << "' field is required to be "
    ...
    // Only driver is a required sub document.
    if (!foundDriver) {
...
Status ClientMetadata::parseApplicationDocument(const BSONObj& doc) {
    BSONObjIterator i(doc);
...
void ClientMetadata::serialize(StringData driverName,
                               ...
                               BSONObjBuilder& builder) {
    dassert(!driverName.empty() && !driverVersion.empty() && !osType.empty() && !osName.empty() &&
