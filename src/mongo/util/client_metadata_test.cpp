...
namespace mongo {
const StringData kMetadataDoc = "meta";
...
TEST(ClientMetadatTest, TestRequiredOnlyFields) {
    // Without app name
    ASSERT_DOC_OK(kDriver << BSON(kName << "n1" << kVersion << "v1"));
...
TEST(ClientMetadatTest, TestNegativeMissingRequiredOneField) {
    ASSERT_DOC_NOT_OK(kDriver << BSON(kVersion << "v1"));
    ASSERT_DOC_NOT_OK(kDriver << BSON(kName << "n1"));
}
