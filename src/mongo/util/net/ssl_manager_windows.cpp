...
std::string getCertificateSubjectName(PCCERT_CONTEXT cert) {
    DWORD needed =
        CertNameToStrW(cert->dwCertEncodingType,
                       ...
                       0);
    uassert(
        50750, str::stream() << "CertNameToStr size query failed with: " << needed, needed != 0);
...
StatusWith<UniqueCertificate> loadCertificateSelector(SSLParams::CertificateSelector selector) {
...
Status SSLManagerWindows::_loadCertificates(const SSLParams& params) {
    ...
    if (hasCertificateSelector(params.sslCertificateSelector)) {
        auto swCert = loadCertificateSelector(params.sslCertificateSelector);
        if (!swCert.isOK()) {
            return swCert.getStatus();
        }
    ...
    if (hasCertificateSelector(params.sslClusterCertificateSelector)) {
        auto swCert = loadCertificateSelector(params.sslClusterCertificateSelector);
        if (!swCert.isOK()) {
            return swCert.getStatus();
        }
...
constexpr auto OneHundredNanosecondsSinceEpoch = 116444736000000000LL;
...
unsigned long long FiletimeToEpocMillis(FILETIME ft) {
    uint64_t ns100 = ((static_cast<int64_t>(ft.dwHighDateTime) << 32) + ft.dwLowDateTime) -
        OneHundredNanosecondsSinceEpoch;
    return ns100 / 1000;
}
...
Status validatePeerCertificate(const std::string& remoteHost,
    ...
    // Ignore CRYPT_E_NO_REVOCATION_CHECK since most CA lack revocation information especially test
    // certificates
