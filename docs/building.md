...
In order to build MongoDB, Python 2.7.x is required, and the several Python modules. To install
the required Python modules, run:

    $ pip2 install -r buildscripts/requirements.txt

Note: If `pip2` common is not available, the `pip` may be the pip command associated with Python
2.7.x
