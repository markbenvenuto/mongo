...
(function() {
    ...
    let c = coll.count( { '$where': function() { sleep(1000); return true} } );
    ...
    assert(/COMMAND .* command test.foo appName:MongoDB Shell command: count { count: "foo", query: { \$where: function ()/.test(log), "received client metadata log line missing in mongod log file!");
